(() => {
  ...
  function validateTarget(target, expectedText, deps) {
    ...
    if (typeof deps.resolveButton === "function" && deps.resolveButton(context) !== button) return {ok:false,code:"SEND_TARGET_CHANGED"};
  ...
  function clickSynchronously({ target, expectedText, deps, beforeClick = null }) {
    const validation = validateTarget(target, expectedText, deps);
    if (!validation.ok) {const e=new ComposerSendError(validation.code,validation.code);e.method_called=false;e.click_event_observed=false;throw e;}
    const preClickSnapshot = snapshotTarget(target, deps);
    try{if(typeof beforeClick === "function")beforeClick(preClickSnapshot);}catch(e){e.method_called=false;e.click_event_observed=false;throw e;}
