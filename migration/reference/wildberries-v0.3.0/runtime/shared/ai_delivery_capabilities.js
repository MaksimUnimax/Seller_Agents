(() => {
  ...
  function fileDispatchDecision(adapterId, descriptor = {}) {
    ...
    const extension = String(descriptor.extension || extensionFromFilename(descriptor.filename)).toLowerCase();
    const byteLength = Math.max(0, Number(descriptor.byte_length || descriptor.byteLength || 0));
    const mimeType = String(descriptor.mime_type || descriptor.mimeType || descriptor.mime || "").split(";", 1)[0].trim().toLowerCase();
    ...
    const integrityBackedOriginalProviderFile = sourceKind === "original_provider_file" && artifactKey.startsWith("provider:") && /^[a-f0-9]{64}$/.test(sha256);
    const typeIsConcrete = Boolean(extension && extension !== "bin" && mimeType && mimeType !== "application/octet-stream");
    if (staticSupport.reason === "file_type_not_supported" && runtimePolicy === "runtime_target_verification" && integrityBackedOriginalProviderFile && typeIsConcrete) {
      return Object.freeze({ status: "runtime_verification_required", supported: false, dispatch_allowed: true, runtime_verification_required: true, reason: "target_runtime_verification_required", extension, byte_length: byteLength, mime_type: mimeType, source_kind: sourceKind });
    }
  ...
  function generatedTextDecision(adapterId, text) {
    ...
    return Object.freeze({ representation: thresholdChars > threshold ? "text_document" : "plain_text", threshold_status: "calibrated", unicode_chars: unicodeChars, threshold_chars: thresholdChars, length_metric: metric, threshold });
  }

  globalThis.WBAIDeliveryCapabilities = Object.freeze({
    TARGET_AI_IDS,
    PROFILES,
