(() => {
  ...
  function createFile(bytes, descriptor = {}) {
    ...
    const filename = String(descriptor.filename || "wb-bridge-file.bin");
  ...
  function dispatchFileDrop(target, files) {
    ...
    return Object.freeze({ dispatched: 3, file_count: actual.length, files: Object.freeze(actual.map((file) => Object.freeze({ name: file.name, size: file.size, type: file.type || "" }))) });
  }
  globalThis.WBWebFileAttachment = Object.freeze({
    dispatchFileDrop,
    base64ToBytes,
    createFile,
    inputAcceptsFiles,
