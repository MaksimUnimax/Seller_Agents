(() => {
  ...
  function nextMarker(source, fromIndex, markers) {
    ...
    for (const marker of markers) {
      ...
      if (
        ...
        (index === selected.index &&
          marker.prefix.length > selected.prefix.length)
      ) {
  ...
  function envelopeSlice(source, marker, markers) {
    ...
    while (
      ...
      IGNORABLE_SEPARATOR.test(source[objectStart])
    )
      objectStart += 1;
  ...
  function discover(text, options = {}) {
    ...
    const helpPrefixV1 = String(options.helpPrefixV1 || "");
    const helpPrefixV2 = String(options.helpPrefixV2 || "");
    const apiDiscover = options.apiDiscover;
    const parseHelp = options.parseHelp;
    if (typeof apiDiscover !== "function")
      throw Object.assign(new Error("apiDiscover is required"), {
        code: "MIXED_BATCH_API_DISCOVERY_REQUIRED",
      });
    if (typeof parseHelp !== "function")
      throw Object.assign(new Error("parseHelp is required"), {
        code: "MIXED_BATCH_HELP_PARSER_REQUIRED",
      });
    ...
    if (
      ...
      !helpPrefixV1 ||
      !helpPrefixV2 ||
      new Set([commandPrefix, helpPrefixV1, helpPrefixV2]).size !== 3
    )
      throw Object.assign(
        new Error("Three distinct protocol prefixes are required"),
    ...
    while (cursor < source.length) {
      ...
      if (marker.kind === "api") {
        ...
        const discovery = Array.isArray(rows)
          ? rows.find((row) => Number(row?.marker_index) === 0)
          : null;
        if (!discovery) {
          discovered.push(
            Object.freeze({
              ...
              discovery: Object.freeze({
                ...
                message: String(
                  options.discoveryFailureMessage ||
                    `${commandPrefix} marker could not be converted into a command discovery record.`,
                ),
              }),
            }),
          );
        ...
        try {
          ...
          help = Object.freeze({
            ok: false,
            code: String(error?.code || "HELP_DISCOVERY_FAILED_CLOSED"),
          });
        }
        discovered.push(
          Object.freeze({
  ...
  globalThis.SellerAgentsMixedBatchDiscovery = Object.freeze({ discover });
})();
