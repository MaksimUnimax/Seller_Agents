...
test("P3.6 rejects a tampered real cache while transport is unavailable", async ({
  ...
  const originalSignature = store.record!.envelope.signature;
  const replacementPrefix = originalSignature[0] === "A" ? "B" : "A";
  const tamperedSignature = replacementPrefix + originalSignature.slice(1);
  expect(tamperedSignature).not.toBe(originalSignature);
  expect(tamperedSignature.length).toBe(originalSignature.length);
  store.record!.envelope.signature = tamperedSignature;
  transport.offline();
  expect(await extension.bootstrapWithPolicy(policyRequest)).toMatchObject({
    kind: "UNAVAILABLE",
