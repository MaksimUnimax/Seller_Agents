...
const candidate=path.resolve(process.argv[2]), out=path.resolve(process.argv[3]), base=path.join(path.dirname(new URL(import.meta.url).pathname),'fixtures/wb024_provider');
...
function load(root,provider=true){
 ...
 for(const file of (provider?['wb_operations','wb_contract','wb_credentials','provider_transport_core','response_verifier','wb_provider']:['wb_operations','wb_contract','wb_credentials']))vm.runInContext(fs.readFileSync(path.join(root,'shared',file+'.js'),'utf8'),context,{filename:file+'.js',timeout:3000});
 return {context,calls,c:context.WBContract,provider:context.WBProvider};
}
const old=load(base,false), cur=load(candidate);const canonical=o=>'WB_API_V1\n'+JSON.stringify(o);
...
await test('024-STRICT-TOPLEVEL-QUERY-REJECTION-PRESERVED',()=>{
 const bad='WB_API_V1\n{"operation":"tariff_box","query":{"injected":"yes"}}';
 assert.throws(()=>old.c.parseCommand(bad),e=>e.code==='UNKNOWN_COMMAND_FIELD');
 assert.throws(()=>cur.c.parseCommand(bad),e=>e.code==='UNKNOWN_COMMAND_FIELD');
 return {baseline_024:'REJECTED',candidate:'REJECTED'};
});
const negatives=[
