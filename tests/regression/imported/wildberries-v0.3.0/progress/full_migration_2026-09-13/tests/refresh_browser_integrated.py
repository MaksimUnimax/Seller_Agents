...
class Fixture:
 async def start(self,browser,ai='chatgpt',new=False):
  ...
  async def send(message):
   ...
   return await self.rpc('call',message=message,sender='content',identity=identity)
...
async def main():
 async with async_playwright() as p:
  ...
  async def ready(f):
   q=await f.begin();expect(q['ok'],str(q));await f.wait_phase('waiting_response');await f.complete();await f.wait_phase('completed');expect(await f.page.evaluate('sentClicks')==1)
  async def action(f,action):
   ident=await f.page.evaluate('()=>({origin:fixtureLocation.origin,ai_id:fixtureAI,conversation_id:fixtureId,status:"confirmed"})')
   return await f.rpc('call',message={'type':'WB_WORK_ACTION','action':action,'tab_id':1,'identity':ident},identity=ident)
  async def check(f,hidden=False):
   await ready(f)
   if hidden:expect((await action(f,'toggle'))['ok'])
   old=await f.page.evaluate('()=>fixtureFire({type:"WB_GET_IDENTITY"})')
   q=await action(f,'refresh');expect(q['ok'],str(q));cur=await f.page.evaluate('()=>fixtureFire({type:"WB_GET_IDENTITY"})')
   expect(cur['runtime_id']!=old['runtime_id'],'Physical runtime not renewed')
   expect(q['work']['state']==('active_hidden' if hidden else 'active_visible'),str(q))
   st=await f.state();rec=[v for k,v in st['data'].items() if k.startswith('wb_work_recovery_v1:')][0]
   expect(rec['phase']=='completed');expect(rec['new_runtime_generation']==cur['runtime_generation'])
   expect(await f.page.evaluate('sentClicks')==1,'Refresh sent initial prompt again')
   expect(sum(m['type']=='WB_RUNTIME_REFRESH' for m in st['messages'])==1,'Repeated reinit')
   expect(await f.page.evaluate('fixtureListeners.length')==1,'Duplicate content listeners')
  async def finish(f):
   await check(f);q=await action(f,'finish');expect(q['ok'],str(q));expect(q['work']['state']=='inactive');expect(await f.page.evaluate('sentClicks')==1)
  async def wake(f):
   await check(f);await f.rpc('recreate');q=await action(f,'refresh');expect(q['ok'],str(q));expect(await f.page.evaluate('sentClicks')==1)
  async def repeat(f):
   await check(f);q=await action(f,'refresh');expect(q['ok'],str(q));expect(await f.page.evaluate('fixtureListeners.length')==1);expect(await f.page.evaluate('sentClicks')==1)
  async def draft(f):
   await ready(f);await f.page.locator('textarea').fill('Неотправленный текст');q=await action(f,'refresh');expect(q['ok'],str(q));expect(await f.page.locator('textarea').input_value()=='Неотправленный текст');expect(await f.page.evaluate('sentClicks')==1)
  for ai in ['chatgpt','alice']:
   for name,fn in [('REFRESH',check),('HIDDEN',lambda f:check(f,True)),('FINISH',finish),('WORKER_RECREATION',wake),('REPEATED_REFRESH',repeat),('DRAFT_PRESERVED',draft)]:
    if os.environ.get('CASE') and os.environ['CASE'] not in ai+'_'+name:continue
    f=None
    try:
     f=await Fixture().start(browser,ai=ai);await fn(f);expect(not f.errors,repr(f.errors));st=await f.state();expect(st['counts']['provider']==0,'Provider fetch');emit(ai+'_'+name,True)
    except Exception as e:
     emit(ai+'_'+name,False,{'error':str(e),'traceback':traceback.format_exc()})
     if f:
      diag={'rpc_reader_done':f.pump.done(),'rpc_reader_exception':str(f.pump.exception()) if f.pump.done() else None}
      (out/(ai+'_'+name+'.diagnostic.json')).write_text(json.dumps(diag,indent=2))
      try:(out/(ai+'_'+name+'.state.json')).write_text(json.dumps(await f.state(),ensure_ascii=False,indent=2))
      except Exception as de:(out/(ai+'_'+name+'.diagnostic-error.txt')).write_text(str(de))
      try:await f.page.screenshot(path=str(out/(ai+'_'+name+'.png')))
      except Exception:pass
    finally:
     if f:await f.close()
  await browser.close()
 summary={'passed':sum(x['status']=='PASS' for x in rows),'failed':sum(x['status']=='FAIL' for x in rows),'scope':'Real content plus real worker in Chromium; synthetic AI DOM and mocked Chrome transport/storage; no real provider; not native MV3 or installed acceptance'}
