...
async def main():
 async with async_playwright() as p:
  ...
  async def wait_saved(f):
   q=await f.rpc('call',message={'type':'WB_SAVE_RUNTIME_OPTIONS','options':{'composer_wait_ms':1000}});expect(q['ok'],str(q))
   await f.page.locator('textarea').fill('Черновик пользователя');await f.execute(FILE);await asyncio.sleep(1.4)
   op=await f.operation();expect(op.get('status')=='delivering',str(op.get('status')));expect(await f.page.locator('textarea').input_value()=='Черновик пользователя');expect(await f.page.evaluate('fileApplications')==0)
  async def clear(f):
   await wait_saved(f);await f.page.locator('textarea').fill('');op=await f.terminal();expect(op['delivery_confirmed']);expect(await f.page.evaluate('fileApplications')==1);expect((await f.state())['counts']['provider_total']==1)
   expect(not any(m['type']=='WB_LOCAL_COMMAND_ERROR' for m in (await f.state())['messages']))
  async def replaced(f):
   await wait_saved(f);await f.page.locator('textarea').evaluate('(n)=>{const c=n.cloneNode(true);c.value="";n.replaceWith(c);}');op=await f.terminal();expect(op['delivery_confirmed']);expect((await f.state())['counts']['provider_total']==1)
  async def finish(f):
   await wait_saved(f);identity=await f.page.evaluate('()=>({origin:fixtureLocation.origin,ai_id:fixtureAI,conversation_id:fixtureId,status:"confirmed"})');q=await f.rpc('call',message={'type':'WB_WORK_ACTION','action':'finish','tab_id':1,'identity':identity},identity=identity);expect(q['ok'],str(q));await f.page.locator('textarea').fill('');await asyncio.sleep(.5);expect(await f.page.evaluate('sentClicks')==1);expect(await f.page.evaluate('fileApplications')==0);expect((await f.state())['counts']['provider_total']==1)
  for ai in ['chatgpt','alice']:
   for name,fn in [('CLEAR_RESUMES_SAVED_FILE',clear),('REPLACED_COMPOSER',replaced),('FINISH_CANCELS_WAIT',finish)]:
    f=None
    try:f=await FileFixture().start(browser,ai=ai);await fn(f);expect(not f.errors,repr(f.errors));emit(ai+'_'+name,True)
    except Exception as e:emit(ai+'_'+name,False,{'error':str(e),'trace':traceback.format_exc()})
    finally:
     if f:await f.close()
  await browser.close()
 summary={'passed':sum(x['status']=='PASS' for x in rows),'failed':sum(x['status']=='FAIL' for x in rows),'scope':'Actual content/worker/File/DataTransfer in Chromium; fixture provider only; no installed acceptance'}
