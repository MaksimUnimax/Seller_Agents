...
function implementedTargetIds() { return globalThis.OzonAIDeliveryCapabilities?.implementedTargetIds?.() || []; }
function targetLabel(id) { return globalThis.OzonAIDeliveryCapabilities?.targetLabel?.(id) || String(id || "AI"); }
function targetImplemented(id) { return implementedTargetIds().includes(String(id || "")); }
function populateTargetModes() {
  const select=$("aiMode"); if(!select) return;
  for(const option of [...select.querySelectorAll('option')]) if(option.value!=="auto") option.remove();
  for(const id of implementedTargetIds()){ const option=document.createElement("option"); option.value=id; option.textContent=targetLabel(id); select.appendChild(option); }
  if($("supportedAdapters")) $("supportedAdapters").textContent=implementedTargetIds().map(targetLabel).join(" · ") || "—";
}
...
async function resolvePopupContext({ required = true } = {}) {
  ...
  if (!resolved.ok || !resolved.context?.conversation_key) {
    const supportedNewChat = targetImplemented(page.identity?.ai_id) && !page.identity?.conversation_id;
...
function renderState(state) {
  ...
  const selectedAIMode = ["auto", ...implementedTargetIds()].includes(String(state.ai_mode || "")) ? String(state.ai_mode) : "auto";
  $("aiMode").value = selectedAIMode;
  const detectedLabel = targetImplemented(state.detected_ai_id) ? targetLabel(state.detected_ai_id) : "не определён";
  $("aiModeMeta").textContent = selectedAIMode === "auto"
    ...
    : `Override только этой вкладки: ${targetLabel(selectedAIMode)}. Другие вкладки работают независимо; несовпадение с hostname fail-closed.`;
...
$("workStart")?.addEventListener("click", () => busy($("workStart"), async () => {
  ...
  if (!context.tab_id || !identity || !targetImplemented(identity.ai_id)) {
    return status("Текущая вкладка не является поддерживаемым AI-диалогом.", "error");
  }
...
async function refresh() {
  ...
  const state = {
    ...
    work_start_available: Boolean(context.tab_id && context.page?.identity && targetImplemented(context.page.identity.ai_id))
  };
  renderState(state);
...
$("aiMode").addEventListener("change", async () => {
  ...
  status(requested === "auto"
    ...
    : `Эта вкладка: adapter override ${targetLabel(requested)}. Остальные вкладки не изменялись.`, "ok");
});
