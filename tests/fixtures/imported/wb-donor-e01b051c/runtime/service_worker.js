/* global OzonAIDeliveryCapabilities, BB2ConversationIdentity, BB2ManualControls, OzonRuntime, OzonCredentials, OzonOperationRegistry, OzonEntitlements, OzonContract, OzonGuidance, OzonMixedBatchDiscovery, OzonWorkSessionModel, BridgeAutorunModel, ProviderTransportCore, OzonProvider */
importScripts("shared/ai_delivery_capabilities.js", "shared/conversation_identity.js", "shared/runtime_names.js", "shared/work_session_model.js", "shared/manual_controls.js", "shared/ozon_credentials.js", "shared/ozon_operation_registry.js", "shared/ozon_entitlements.js", "shared/ozon_contract.js", "shared/ozon_guidance.js", "shared/bridge_autorun_model.js", "shared/provider_transport_core.js", "shared/ozon_provider.js");
...
const AI_MODES = new Set(["auto", ...(OzonAIDeliveryCapabilities?.implementedTargetIds?.() || [])]);
const SUPPORTED_AI_TAB_URLS = OzonAIDeliveryCapabilities?.implementedContentScriptPatterns?.() || [];
...
async function broadcastCopyButtonProfiles(collection) {
  ...
  const tabs = await chrome.tabs.query({ url: SUPPORTED_AI_TAB_URLS }).catch(() => []);
...
function normalizeIdentity(identity) {
  ...
  if (!aiId) {
    throw Object.assign(new Error("Origin не принадлежит поддерживаемому AI."), { code: "INVALID_AI_ORIGIN" });
  }
  if (status === "conflict") {
...
function expectedDeliveryConfirmationBasis(origin) {
  ...
  return OzonAIDeliveryCapabilities?.deliveryConfirmationBasis?.(provider) || "";
}

function acceptedDeliveryConfirmationBasis(origin, basis) {
  const provider = BB2ConversationIdentity.providerForOrigin(String(origin || "").toLowerCase());
  return (OzonAIDeliveryCapabilities?.acceptedDeliveryConfirmationBases?.(provider) || []).includes(String(basis || ""));
}

function legacyConversationKey(identity) {
  const normalized = normalizeIdentity(identity);
  const prefix = String(OzonAIDeliveryCapabilities?.profile?.(normalized.ai_id)?.legacy_conversation_key_prefix || "");
...
async function bindConversation(context, options = {}) {
  ...
  if (!expected.conversation_id) {
    throw Object.assign(new Error("Нужен подтверждённый диалог поддерживаемого AI с устойчивым conversation ID."), { code: "CONVERSATION_NOT_CONFIRMED" });
  }
...
async function resolveConfirmedConversationKey(identity) {
  ...
  const key = conversationKeyFromIdentity(normalized);
  if (!key) {
    throw Object.assign(new Error("Нужен подтверждённый AI-диалог. Для нового чата сначала должен появиться устойчивый conversation ID и, если профиль цели требует этого, подтверждение активного диалога; до этого Ozon Bridge работает fail-closed."), { code: "CONVERSATION_NOT_CONFIRMED" });
  }
...
function localGuidanceResult(entry) {
  ...
  const payload = OzonGuidance.result({ status, cluster, section, version, error: error.code || classified.error || null, descriptor, validation_message: error.message || null });
...
async function ensureBatchLocalPolicy({ ownerKind, ownerId, getOwner, mutateOwner, ownerMatches, isCollecting, failOwner }) {
  ...
  const fileDeliveryRevision = globalThis.BridgeAutorunModel.targetFileBudget?.(owner) !== null
