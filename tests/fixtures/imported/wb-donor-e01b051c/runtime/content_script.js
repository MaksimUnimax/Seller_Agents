...
(() => {
  ...
  function conversationIdentity() {
    ...
    const adapter = currentAIAdapter();
    const activeConversationId = typeof adapter?.activeConversationId === "function" ? adapter.activeConversationId() : null;
  ...
  function ensureToastRoot() {
    ...
    // Status plates belong at the top so they never cover the target AI composer.
  ...
  function renderOwnButton(record) {
    ...
    record.button.title = ready
      ? `Ozon manual: выполнить команды из этого ${(OzonAIAdapters.ADAPTERS[record.binding.adapter_id]?.label || "AI")} code block`
  ...
  function targetControlStrategy(adapter = currentAIAdapter()) {
    return String(globalThis.OzonAIDeliveryCapabilities?.profile?.(adapter?.id)?.composer_control_strategy || "");
  }

  function adapterPrimaryComposerContext(adapter = currentAIAdapter()) {
    if (!adapter || typeof adapter.composerContext !== "function") return null;
    const context = adapter.composerContext();
    if (!context?.composer || !context?.root || !context.composer.isConnected || !context.root.isConnected || !visible(context.composer)) return null;
    return context;
  }

  function classifyAdapterComposerControl(context = adapterPrimaryComposerContext()) {
    const adapter = currentAIAdapter();
    if (!adapter || !context || typeof adapter.classifyComposerControl !== "function") return { kind: "unknown", context, button: null };
    const classified = adapter.classifyComposerControl(context, visible);
    if (classified?.kind === "ready") return { kind: "target_ready", context, button: classified.button || null };
    return { kind: classified?.kind || "unknown", context, button: classified?.button || null };
  }

  function primaryComposerContext() {
    const adapter = currentAIAdapter();
    const strategy = targetControlStrategy(adapter);
    if (strategy === "adapter_explicit_ready_v1") return adapterPrimaryComposerContext(adapter);
    if (strategy === "legacy_microphone_completion_v1") return chatgptPrimaryComposerContext();
    return null;
  }
  ...
  function allComposerControls(context) {
    const adapter = currentAIAdapter();
    const strategy = targetControlStrategy(adapter);
    if (strategy === "adapter_explicit_ready_v1") {
      const root = context?.root;
      if (!(root instanceof Element) || !root.contains(context?.composer)) return [];
      return [...root.querySelectorAll('button, [role="button"], input[type="submit"]')].filter((button) => button instanceof HTMLElement && visible(button));
    }
    if (strategy === "legacy_microphone_completion_v1") return chatgptAllComposerControls(context);
    return [];
  }
  ...
  function sendButtonCandidates(context) {
    ...
    const strategy = targetControlStrategy(adapter);
    if (strategy === "adapter_explicit_ready_v1") {
      const classified = classifyAdapterComposerControl(context);
      return ["send_active", "send_disabled"].includes(classified.kind) && classified.button ? [{ button: classified.button, score: 2000 }] : [];
    }
    if (strategy === "legacy_microphone_completion_v1") return chatgptSendButtonCandidates(context);
    return [];
  }
  ...
  function sendButton(context) {
    const adapter = currentAIAdapter();
    const strategy = targetControlStrategy(adapter);
    if (strategy === "adapter_explicit_ready_v1") { const classified=classifyAdapterComposerControl(context); return classified.kind === "send_active" ? classified.button : null; }
    if (strategy === "legacy_microphone_completion_v1") return chatgptSendButton(context);
    return null;
  }

  function microphoneButton(context) { return targetControlStrategy() === "legacy_microphone_completion_v1" ? chatgptMicrophoneButton(context) : null; }
  function stopButton(context) { const strategy=targetControlStrategy(); if(strategy === "adapter_explicit_ready_v1"){const c=classifyAdapterComposerControl(context);return c.kind === "stop" ? c.button : null;} return strategy === "legacy_microphone_completion_v1" ? chatgptStopButton(context) : null; }
  function recognizedSendControl(context) { const strategy=targetControlStrategy(); if(strategy === "adapter_explicit_ready_v1"){const c=classifyAdapterComposerControl(context);return ["send_active","send_disabled"].includes(c.kind)?c.button:null;} return strategy === "legacy_microphone_completion_v1" ? chatgptRecognizedSendControl(context) : null; }
  function classifyComposerControl() { const strategy=targetControlStrategy(); if(strategy === "adapter_explicit_ready_v1") return classifyAdapterComposerControl(); if(strategy === "legacy_microphone_completion_v1") return classifyChatgptComposerControl(); return {kind:"unknown",context:null,button:null}; }
  ...
  function diagnosticForControlKind(kind) {
    return {
      ...
      target_ready: "BUTTON_TARGET_READY",
  ...
  async function runBatchDeliveryWatch(recovery) {
    ...
    while (current() && activeDeliveryWatch === state && !state.cancelled) {
      ...
      if (classified.kind === "target_ready") {
        // Explicit-ready targets may confirm only after this delivery's one-shot Send click.
        if (!state.send_click_committed) {
          recordContentDiagnostic("TARGET_READY_BEFORE_DELIVERY_CLICK_IGNORED", { owner_kind: state.owner_kind, owner_id: state.owner_id, delivery_id: state.delivery_id, click_attempts: state.click_attempts });
          await waitForDeliveryWatchWake(state);
          continue;
        }
        const completion = await sendRuntime("OZ_BATCH_DELIVERY_COMPLETE", {
          ...
          delivery_id: state.delivery_id,
          delivery_confirmed: true,
          confirmation_basis: "target_ready",
          click_attempts: state.click_attempts
        });
        if (!completion?.ok) {
          recordContentDiagnostic("DELIVERY_SUCCESS_ACK_FAILED", { owner_kind: state.owner_kind, owner_id: state.owner_id, delivery_id: state.delivery_id, code: completion?.code || "DELIVERY_CONFIRMATION_REJECTED" });
        ...
        recordContentDiagnostic("DELIVERY_SUCCESS", { owner_kind: state.owner_kind, owner_id: state.owner_id, delivery_id: state.delivery_id, click_attempts: state.click_attempts, confirmation_basis: "target_ready" });
        toast("Ozon: объединённый batch-результат доставлен; целевой AI снова готов к вводу.", "success", 5000, "autorun-state");
        stopDeliveryWatch("delivery_success");
        if (state.owner_kind === "manual") {
          setManualBridgeReady(manualEnabled && manualConversationKey === state.conversation_key, "target_ready_confirmed");
        }
        return { ok: true, delivered: true, click_attempts: state.click_attempts };
      }
  ...
  async function deliverReport(reportText, autoSend, { confirmUserTurn = false, runId = "manual", deliveryId = "", requestId = "", requireCommit = false, conversationKey = "" } = {}) {
    ...
    if (existing && !ownsStage && normalizedDeliveryText(existing) !== normalizedDeliveryText(reportText)) {
      throw new Error("В поле целевого AI уже есть неотправленный текст до подготовки отчёта.");
    }
