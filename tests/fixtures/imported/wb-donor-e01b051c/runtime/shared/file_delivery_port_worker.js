...
(() => {
  ...
  function normalizedLiveOwner(value) {
    ...
    const syntheticPath = BB2ConversationIdentity.conversationPathFor?.(provider, conversationId);
    if (!syntheticPath) return null;
  ...
  async function readRetainedText(owner, command) {
    ...
    // refs that do not use local retained-text flow keep their original execution path.
  ...
  async function retainCompleteText(owner, text) {
    ...
    const notice = localFileDeliveryResult(command, { state: "text_retained", reason: "TARGET_AI_TEXT_CAPACITY",
      message: "Полный текст результатов не помещается в сообщение целевого AI и сохранён без сокращений локально, не дольше одного часа от подготовки доставки. Получите TXT отдельной следующей командой; она не повторяет запросы к Ozon.",
  ...
  async function fallbackBeforeAttachment(found, code) {
    const fallbackProfile = OzonAIDeliveryCapabilities.profile(found.owner.delivery?.adapter_id);
    if (fallbackProfile?.pre_attachment_failure_text_fallback !== true || found.owner.delivery?.phase !== PHASES.CLAIMED) return null;
    ...
    let withheld = null;
    let prefixDelivered = found.owner.delivery.report_prefix_applied === true;
    if (OzonAIDeliveryCapabilities.generatedTextDecision(found.owner.delivery.adapter_id, text).representation !== "plain_text") {
      prefixDelivered = false;
