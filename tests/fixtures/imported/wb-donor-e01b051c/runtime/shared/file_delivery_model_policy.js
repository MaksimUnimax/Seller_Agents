...
(() => {
  ...
  function targetFileBudget(owner) {
  ...
  function fileBudgetDecision(owner, entry) {
    const maximum = targetFileBudget(owner);
    ...
    return Object.freeze({
      state: "deferred", reason: "TARGET_AI_FILE_LIMIT", target_ai: globalThis.OzonAIDeliveryCapabilities?.adapterIdForOrigin?.(owner?.origin) || null,
      max_files_per_turn: maximum, external_request_executed: false,
      message: `Целевой AI принимает не более ${maximum} файл(ов) за одно сообщение. Эта файловая команда отложена до следующего сообщения; запрос к Ozon не выполнялся.`,
  ...
  function claimDelivery(run, payload = {}) {
    ...
    if (providerFileRefs.length && targetFileBudget(run) !== null && globalThis.OzonAIDeliveryCapabilities?.profile?.(next.delivery.adapter_id)?.inline_mixed_text_with_files === true) {
