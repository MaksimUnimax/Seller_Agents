(() => {
  ...
  const AI_MODES = Object.freeze(["auto", ...(globalThis.OzonAIDeliveryCapabilities?.implementedTargetIds?.() || [])]);
  ...
  function aliceAttachFiles(surface, files) {
    if (surface?.kind !== "drag_drop_v1" || surface.root !== document.body || !surface.root?.isConnected) {
      throw Object.assign(new Error("Target AI drag-drop attachment surface is unavailable."), { code: "TARGET_AI_ATTACHMENT_SURFACE_UNAVAILABLE" });
    }
    ...
    if (!live || live.root !== surface.root || !surface.capability_marker?.isConnected || live.capability_marker !== surface.capability_marker) {
      throw Object.assign(new Error("Target AI drag-drop capability marker changed before attachment dispatch."), { code: "TARGET_AI_ATTACHMENT_SURFACE_CHANGED" });
    }
    const helper = globalThis.OzonWebFileAttachment;
    if (typeof helper?.dispatchFileDrop !== "function") throw Object.assign(new Error("Target AI drag-drop browser primitive is unavailable."), { code: "TARGET_AI_ATTACHMENT_TRANSPORT_UNAVAILABLE" });
  ...
  const CHATGPT = Object.freeze({
    ...
    activeConversationId() { return null; },
    deliveryCapabilities() { return globalThis.OzonAIDeliveryCapabilities?.profile?.(this.id) || null; },
  ...
  function activeHistoryConversationId() {
  ...
  const ALICE = Object.freeze({
    ...
    activeConversationId() { return activeHistoryConversationId(); },
    deliveryCapabilities() { return globalThis.OzonAIDeliveryCapabilities?.profile?.(this.id) || null; },
