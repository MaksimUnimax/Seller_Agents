...
(() => {
  ...
  function extensionOnlyOperation(normalized) {
    return !Object.prototype.hasOwnProperty.call(patchedBaseContract.OPERATIONS, normalized.operation);
  }

  function extensionAwareAnalyticsCoalescingDescriptor(command) {
    const normalized = normalizeCommand(command);
    if (!extensionOnlyOperation(normalized)) return patchedBaseContract.analyticsCoalescingDescriptor(normalized);
    ...
    fail("ANALYTICS_COALESCING_EXTENSION_UNSUPPORTED", "Extension-only analytics_data operation requires an explicit coalescing contract.");
  }

  function extensionAwareBuildAnalyticsCoalescedCommand(commands) {
    const list = Array.isArray(commands) ? commands : [];
    if (list.length < 2) return patchedBaseContract.buildAnalyticsCoalescedCommand(list);
    const normalized = list.map((command) => normalizeCommand(command));
    if (normalized.some((command) => extensionOnlyOperation(command))) {
      if (normalized.some((command) => command.operation !== "analytics_data")) {
        fail("ANALYTICS_COALESCING_INELIGIBLE", "Coalescing разрешён только для analytics_data; extension-only read operations не объединяются.");
      }
      fail("ANALYTICS_COALESCING_EXTENSION_UNSUPPORTED", "Extension-only analytics_data operation requires an explicit coalescing contract.");
    }
    return patchedBaseContract.buildAnalyticsCoalescedCommand(normalized);
  }

  function extensionAwareReviewedAnalyticsAcquisitionProfile(command) {
    const normalized = normalizeCommand(command);
    if (!extensionOnlyOperation(normalized)) return patchedBaseContract.reviewedAnalyticsAcquisitionProfile(normalized);
    if (normalized.operation !== "analytics_data") {
    ...
    fail("ANALYTICS_ACQUISITION_EXTENSION_UNSUPPORTED", "Extension-only analytics_data operation requires an explicit acquisition profile.");
  }

  function sellerCapabilityRequirement(command, atMs = Date.now(), entitlementSnapshot = null) {
    const normalized = normalizeCommand(command);
    if (normalized.operation !== "notification_check") {
      if (!extensionOnlyOperation(normalized)) return patchedBaseContract.sellerCapabilityRequirement(normalized, atMs, entitlementSnapshot);
      ...
      if (provider !== "seller_api") {
        return deepFreeze({
          required: false,
          known: true,
          reasons: [provider === "performance_api" ? "performance_provider_not_seller_subscription" : "non_seller_provider"],
          allowed_subscription_types: [],
          entitlement_key: meta.entitlement_key || (String(meta.method || "") + " " + String(meta.path || "")),
          rule_source: null,
          default_access: "ALL_ACCOUNTS"
        });
      }
      fail("SELLER_CAPABILITY_EXTENSION_RULE_MISSING", "Extension-only Seller operation " + normalized.operation + " requires an explicit capability rule.");
    }
  ...
  const patchedContract = Object.freeze({
    ...
    commandFingerprint,
    analyticsCoalescingDescriptor: extensionAwareAnalyticsCoalescingDescriptor,
    buildAnalyticsCoalescedCommand: extensionAwareBuildAnalyticsCoalescedCommand,
    reviewedAnalyticsAcquisitionProfile: extensionAwareReviewedAnalyticsAcquisitionProfile,
  ...
  if (baseGuidance) {
    ...
    function result({ status = "cluster_required", cluster = null, section = null, error = null, descriptor = null, version = 1 } = {}) {
      ...
      return Object.freeze({
        bridge: "ozon-llm-api-bridge", version: globalThis.OzonRuntime?.RUNTIME?.version || "0.1.22", guidance_version: String(version >= 2 ? 2 : 1), status,
