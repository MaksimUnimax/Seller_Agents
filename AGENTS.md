...
D1.E1 открыт новым поручением владельца «Делай» после отчёта D1.E0. Разрешены перенос закреплённых Ozon/WB исходников и тестов по карте, минимальная адаптация путей/упаковки/CI, проверка и публикация переноса в Seller_Agents. Ozon runtime переносится без изменения байтов; WB сохраняется как reference с INSTALLED FAIL. Сервер, его контракты и исходные ветки не менять. D2, live provider tests и browser-store publication этим этапом не выполняются. Текущие результаты: docs/migration/evidence/extension-import-2026-09-14/README.md; команды: docs/development/EXTENSION_BASELINE.md.
