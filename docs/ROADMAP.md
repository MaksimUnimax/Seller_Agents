...
> **Текущий forward-only cursor, 2026-09-18.** Локальная линия Early I1 уже продвинулась дальше удалённого integration: отчёт текущего C2.3-C3A marketplace-predispatch шага получен, но его архитектурная приёмка остановлена владельцем до исправления автономной модели. Следующий обязательный участок после обработки C3A — вставленная ниже линия C2.3-C3B→C3H. Она должна быть завершена **до** provider replay / joint offline command-result recovery. Уже выполненные C1/C2/C3A этапы не удаляются и не откатываются; они остаются историческими bounded-шагами, а C3B→C3H фундаментально исправляет ошибочно суженную автономность и затем возвращает поток к прежней последовательности.
