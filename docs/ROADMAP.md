...
C2.2-A is closed only for online-first verified cached-bootstrap configuration acquisition with the durable effective-time floor, signed cache re-verification, context binding and ownership/obsolescence fences. C2.2-B is closed only for read-only projection of those already verified signed `features`, `entitlements`, and signed AI metadata; it always exposes `executionAuthority: false` and does not create `capabilities` or `workAllowed`. Neither acceptance authorizes offline Work, executable profile/capability authority, provider replay, joint offline command-result behavior or scheduler integration. Before any signed feature/entitlement can become executable authority, a later bounded step must define an independent packaged local capability authority and intersect it with the signed permission. Health, S1.2/D3, full I1/D2, the Octoport domain migration, beta, deployment and release remain separate open work.
