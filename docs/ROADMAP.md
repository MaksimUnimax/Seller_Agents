...
C2.2-A is closed only for online-first verified cached-bootstrap configuration acquisition with the durable effective-time floor, signed cache re-verification, context binding and ownership/obsolescence fences. C2.2-B is closed only for read-only projection of those already verified signed `features`, `entitlements`, and signed AI metadata; it always exposes `executionAuthority: false` and does not create `capabilities` or `workAllowed`. C2.2-C is closed only for the independent immutable packaged-local capability-presence authority: the current package records Ozon, Wildberries, ChatGPT web and Alice web adapter presence, with `executionAuthority: false` and zero signed permission bindings. None of these acceptances authorize offline Work, signed-permission→capability mapping, effective executable capability authority, provider replay, joint offline command-result behavior, or scheduler integration. The next bounded C2 step must identify explicit reviewed signed server permission keys and define a fail-closed mapping/intersection with the packaged authority; unknown/missing permission cannot activate a local capability, remote allow cannot manufacture an absent package capability, and denial must win. Prefer keeping that next result read-only/non-executing until its own acceptance is complete. Health, S1.2/D3, full I1/D2, the Octoport domain migration, beta, deployment and release remain separate open work.
