...
S1.1 free beta eligibility и quota регистраций приняты. Непосредственный приоритет Stream A — ранняя серверная половина I1: сначала `I1-SRV.0` (current I1 contract / dependency audit), затем `I1-SRV.1` (signed canonical account identity), после чего оставшиеся шаги I1 выполняются последовательно только после принятия предыдущих вплоть до `I1-SRV.5`.
