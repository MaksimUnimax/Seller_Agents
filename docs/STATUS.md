...
I1-SRV.0 AUDIT COMPLETE; I1-SRV.1 IMPLEMENTED CANDIDATE; OWNER_ARCHITECT_REVIEW_PENDING. Кандидат на ветке `feature/server-i1-auth-bootstrap` добавляет стабильный канонический Seller Agents account UUID в существующий подписанный bootstrap payload. D2.4 остаётся `DEVELOPMENT_APPLICATION_VERIFIED`, `REAL_ACCOUNT_AUTH_NOT_CONNECTED` сохраняется; I1-SRV.2 и последующие шаги, S1.2 и интеграция runtime не начаты.
