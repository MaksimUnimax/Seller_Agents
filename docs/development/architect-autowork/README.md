...
BROWSER_AUTOMATION_EXECUTION_POLICY.md — обязательный companion для всех extension/browser задач. Он фиксирует доказанный server-side Playwright путь через bundled Chromium + persistent context + реальную unpacked сборку, запрещает считать branded Chrome эквивалентом bundled Chromium, запрещает выводить Opera/Yandex PASS из Chromium, требует отдельный Firefox/WebExtension путь, не позволяет выдавать browser restart за forced idle suspension и требует полностью исчерпать Codex/server automation до единого owner-session/manual batch.
