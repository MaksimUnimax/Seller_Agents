...
async function assertDeniedAttachments(worker, owner, idb, beforeReads) {
  ...
  for (const type of ["OZ_ATTACHMENT_RECOVERY_GET", "OZ_ATTACHMENT_COMMIT", "OZ_ATTACHMENT_SEND_COMMIT", "OZ_ATTACHMENT_ARTIFACT_META", "OZ_ATTACHMENT_ARTIFACT_CHUNK"]) {
    const result = await worker.portRequest({ type, ...fields, artifact_key: [...idb.records.keys()][0] || "provider:r5-denied-artifact", offset: 0, length: 32 });
    results.push({ type, result });
    assert.equal(result.ok, false, `${type} must reject`);
    assert.equal(result.code, "WORK_POLICY_BLOCKED", `${type} must reject for authority`);
    for (const field of ["attach_allowed", "click_allowed", "insert_allowed"]) assert.notEqual(result[field], true);
...
{
  const bytes = new Uint8Array([37, 80, 68, 70, 45, 49, 10, 0, 255]);
  let binaryDenied = false;
  const worker = await makeWorker(runtime, { indexedDB: fakeIDB(), fetch: async url => url.startsWith("https://") ? new Response(bytes, { headers: { "content-type": "application/pdf", "content-disposition": 'attachment; filename="r5-positive.pdf"' } }) : binaryDenied ? response({error:{code:"R5_ARCHITECT_DENIAL"}},403) : response({ ok: true }) });
  try {
    worker.setDialogue("11111111-1111-4111-8111-111111111111");
    ...
    binaryDenied = true;
    await assert.rejects(worker.call("SellerAgentsControlClient.bootstrap"), /R5_ARCHITECT_DENIAL/);
    await assertDeniedAttachments(worker, owner, worker.idb, worker.idb.stats.reads);
    for (const type of ["SA_AUTH_STATE","SA_POPUP_STATE"]) assert.equal((await worker.popup({type,tab_id:worker.tabId})).ok,true);
    console.log(JSON.stringify({architect_post_positive_binary_denial:"PASS", real_artifact_key:true, exact_authority_error:true}));
  } finally { worker.close(); }
}
