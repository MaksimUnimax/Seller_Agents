...
const e2eConfigSigningPairs = process.env.TEST_WORKER_INDEX === undefined ? [
  { keyId: "e2e-config-k1", pair: generateKeyPairSync("ed25519") },
  { keyId: "e2e-config-k2", pair: generateKeyPairSync("ed25519") },
] : [];
const e2eConfigSigningRingJson = e2eConfigSigningPairs.length === 0 ? undefined : JSON.stringify({
  version: 1,
...
if (e2eConfigSigningPairs.length > 0) process.env.CONFIG_SIGNING_PUBLIC_KEY_RING_JSON = JSON.stringify(
...
if (!process.env.CONFIG_SIGNING_PUBLIC_KEY_RING_JSON) throw new Error("E2E worker public signing ring is absent");
