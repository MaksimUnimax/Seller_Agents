...
await namedCase("Q3-A-first-floor-both-storage-fail-and-idb-retention", async () => {
  ...
  try {
    ...
    fail = false; clock.wall = T0 - 100; clock.mono = 1100;
    const inMemoryRecovery = await policy(fixture.worker, { detectedAi: CHATGPT });
    assert.equal(inMemoryRecovery.source, "CACHE");
    ...
    try {
      ...
      assert.equal(idb.stats.writes, 1);
...
for (const later of ["replacement", "forbidden"]) await namedCase("Q5-B-held-cached-verification-" + later, async () => {
  ...
  const fixture = await prepared({ clock, backing, beforeCryptoVerify: async () => {
    ...
    return signed(backing, { ...base, account: { id: "99999999-9999-4999-8999-999999999999", status: "ACTIVE" }, configVersion: 2, serverTime: new Date(T0 + 1).toISOString() });
  } });
  try {
    held.armed = true;
    ...
    const bRecord = clone(backing.local[AUTH]);
    if (later === "replacement") assert.notEqual(bRecord.authority.payload.account.id, aRecord.authority.payload.account.id);
    releaseVerify();
    ...
    assert.deepEqual(backing.local[AUTH], bRecord);
