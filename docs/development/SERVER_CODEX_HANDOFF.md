...
На feature-ветке S1.1 имеет статус `IMPLEMENTED_CANDIDATE`; `OWNER_ARCHITECT_REVIEW_PENDING`. После независимого принятия следующая серверная задача — S1.2 (реальная email-доставка/preprod).
...
Использовать тестовые данные. Настоящую почту и препрод готовить отдельной следующей задачей S1.2.
