...
D2.1 принят; D2.2 развивает общую очередь и контекст: [общие модули и composed-сборка](EXTENSION_CORE.md). Baseline-команды этого документа по-прежнему проверяют неизменённые imported sources; новые модули проверяет отдельный core job.
