...
Status: `S02_SUBMITTED__WAITING_COLLECT_S02`.
...
After the deferred due-time guard, exactly one:

`SEARCH_ASYNC_BATCH_API_V1 {"action":"collectN","jobId":"octoport-serp-s02-20260916","count":1}`

Do not issue another `start` or `submitN` for S02. If `collectN` returns local `NO_DUE_OPERATIONS`, preserve/read back it and wait; if it returns provider-backed success, preserve/read back before export.
...
- next action: `S02 collectN count=1` when due.
