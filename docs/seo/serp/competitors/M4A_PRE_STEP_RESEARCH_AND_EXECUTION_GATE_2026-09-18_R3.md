...
### R06 current-authority override — mandatory

R06 is the one historical transport exception discovered by R2 preflight.
...
Work MUST reconstruct R06 only from the seven recovery chunks named by that manifest and verify:

```text
standard gzip validation = PASS
source bytes = 73385
...
The superseded `docs/seo/serp/raw/R06_08_EXPORT_MANIFEST_2026-09-17.md` is historical incident evidence only and MUST NOT be used as current exact-byte authority.

If the recovery authority does not verify exactly, HOLD. Do not repair it inside Work.
...
M4 = CURRENT
M4A_R3_PREPARED = true
M4A_R3_WORK_STARTED = false
M4A_R3_ACCEPTED = false
M4B_ALLOWED = false



## 18. R3 revalidation after R06 recovery

R2 correctly stopped at input-integrity HOLD and produced no analytical deliverables.

Main Chat then:
- reproduced the historical R06 gzip failure;
- proved the raw DEFLATE body intact;
- found no independent historical full-source Git blob;
- materialized a provider-free derived recovery transport;
- remote-read back all recovery chunks/manifest/QA;
- accepted R06 derived recovery authority;
- executed zero new provider calls.

Methodology, the 15-query set, 300-row expectation and 105-pair contract are unchanged from R2.

Fresh external M4 method research from the same 2026-09-18 preparation remains current; this R3 revision changes input authority only.
